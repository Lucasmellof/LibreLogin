...
public class PaperListeners extends AuthenticListeners<PaperLibreLogin, Player, World> implements Listener {
	...
	public void onJoin(PlayerJoinEvent event) {
		if (plugin.fromFloodgate(event.getPlayer().getName())) return;

		var data = readOnlyUserCache.getIfPresent(event.getPlayer().getUniqueId());
		if (data == null) {
			event.getPlayer().kick(Component.text("Internal error, please try again later. Error:  \"Data is null\""));
			return;
		}
	...
	public void onPreLogin(AsyncPlayerPreLoginEvent event) {
		...
		if (Bukkit.getPlayer(event.getPlayerProfile().getName()) != null) {
